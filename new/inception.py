...
def get_inception_conv_layer1(input_img):
    ...
    tower_1 = Conv2D(4, (1, 1), activation='relu', padding='same')(input_img)
    tower_1 = Conv2D(36, (3, 3), activation='relu', padding='same')(tower_1)

    tower_2 = Conv2D(4, (1, 1), activation='relu', padding='same')(input_img)
...
def get_final_model():
    ...
    FullyConnect = Flatten()(first)
    FullyConnect = model_nn2(FullyConnect, 200, 5, dropout=0.3, batch_normalization=True, activation='relu', neurons_decay=0, starting_power=1, l2=10**-5)
